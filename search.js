...
function findAndSetPathway(dict, item, id_selector) {
  ...
  for (path_data in dict.nodes) {
    ...
    console.log(pathway["data"]["compounds"]);
    if (
...
var MAPLink = $.getJSON("KGML/result/MAPLinkJS.json", function(MAPLink) {
  console.log(MAPLink); // this will show the info it in firebug console
  return MAPLink;
});
...
$(".button6").click(function(){
   var path1=$('#searchbar-pathway').find(":selected").attr("value");
   var path2=$('#searchbar2-pathway').find(":selected").attr("value");
   var href="./graph.html?path1="+path1+"&path2="+path2+"&directed=true";
